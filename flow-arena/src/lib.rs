...
mod tests {
    ...
    fn make_flow(aloud: bool) -> FlowEntity {
        ...
        wrapper("Devote", flow.devote(obj_vec[1].id(), &root, 0).is_ok(), &flow, aloud);
        wrapper("Devote", flow.devote(obj_vec[2].id(), &root, 0).is_ok(), &flow, aloud);
        wrapper("DevotePush", flow.devote_push(obj_vec[3].id(), &root).is_ok(), &flow, aloud);
