    ...
    FlowNode: Node<Id> + Default + Debug + Clone,
{
    fn dock(
        ...
        if vec.is_empty() {
            ...
            if !flow
                .orphan()
                .iter()
                .fold(true, |is_in, id| is_in && set.contains(id))
            {
        ...
        if flow
            .node_map
            .keys()
            .fold(false, |is, id| is || self.contains_node(id))
        {
            Err(FlowError::ExistDock)
        } else if !self.contains_node(owner) {
            Err(FlowError::NotExistOwner)
        } else {
            self.node_mut(owner)
                .map(|node| node.children_ref_mut().extend(vec));
            let _: Vec<()> = flow
                .node_map
                .iter_mut()
                .map(|(_, node)| node.parent_set(owner.clone()))
                .collect();
    ...
    fn undock_impl(
        ...
        for id in set.iter() {
            let filter: Vec<()> = self
                .node_map
                .values()
                .filter(|node| !set.contains(node.id()) && !(node.id() == obj))
        ...
        self.node_map.retain(|id, _| !set.contains(id));
        self.node_mut(obj)
            .map(|node| node.children_ref_mut().clear());
        self.check_assert();
        Ok((flow, vec))
    }
    ...
    fn snap(&self, obj: &Self::Id) -> Result<(Self, Vec<Self::Id>), FlowError> {
        ...
        let node_map: HashMap<Self::Id, Self::Node> = set
            .iter()
            .cloned()
...
mod tests {
    ...
    fn make_flow(aloud: bool) -> (FlowEntity, Vec<EntityId>) {
        ...
        let obj_vec: Vec<NodeEntity> = (0..21)
            .map(|x| FlowNode::from_id(x.clone().into(), ()))
            .collect();
        ...
        wrapper(
            "Link 2->0",
            flow.link_push(obj_vec[2].id(), obj_vec[0].id()),
            &flow,
            aloud,
        );
        wrapper(
            "Devote 2->0",
            flow.devote_push(obj_vec[2].id(), obj_vec[0].id()),
            &flow,
            aloud,
        );
        wrapper(
            "Devote 3->0",
            flow.devote_push(obj_vec[3].id(), obj_vec[0].id()),
            &flow,
            aloud,
        );
        wrapper(
            "Devote 4->0",
            flow.devote_push(obj_vec[4].id(), obj_vec[0].id()),
            &flow,
            aloud,
        );
        wrapper(
            "Devote 5->3",
            flow.devote_push(obj_vec[5].id(), obj_vec[3].id()),
            &flow,
            aloud,
        );
        wrapper(
            "Link 6->3",
            flow.link_push(obj_vec[6].id(), obj_vec[3].id()),
            &flow,
            aloud,
        );
        wrapper(
            "Devote 7->1",
            flow.devote_push(obj_vec[7].id(), obj_vec[1].id()),
            &flow,
            aloud,
        );
        wrapper(
            "Devote 8->1",
            flow.devote_push(obj_vec[8].id(), obj_vec[1].id()),
            &flow,
            aloud,
        );
        wrapper(
            "Devote 9->1",
            flow.devote_push(obj_vec[9].id(), obj_vec[1].id()),
            &flow,
            aloud,
        );
        // erase
        ...
        wrapper(
            "Devote 10->6",
            flow.devote_push(obj_vec[10].id(), obj_vec[6].id()),
            &flow,
            aloud,
        );
        wrapper(
            "Devote 11->6",
            flow.devote_push(obj_vec[11].id(), obj_vec[6].id()),
            &flow,
            aloud,
        );
        wrapper(
            "Devote 12->11",
            flow.devote_push(obj_vec[12].id(), obj_vec[11].id()),
            &flow,
            aloud,
        );
        wrapper(
            "Link 12->6",
            flow.link_push(obj_vec[12].id(), obj_vec[6].id()),
            &flow,
            aloud,
        );
        wrapper(
            "Link 13->11",
            flow.link_push(obj_vec[13].id(), obj_vec[11].id()),
            &flow,
            aloud,
        );
        wrapper(
            "Link 14->12",
            flow.link_push(obj_vec[14].id(), obj_vec[12].id()),
            &flow,
            aloud,
        );
        if cfg!(debug_assertions) && aloud {
            println!("Checked.");
            flow.check_assert()
        };
        (
            flow,
            obj_vec.into_iter().map(|node| node.id().clone()).collect(),
        )
    }
